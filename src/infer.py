...
from dataloader import en_tok, fr_tok, BOS_IDX, EOS_IDX, PAD_IDX, MAX_LEN
...
def _load_model(ckpt_path: str) -> Transformer:
    """Loads either a checkpoint dict (with 'model_state','hparams') or a raw state_dict."""
...
def greedy_decode(model: Transformer, src: torch.Tensor, max_len: int) -> torch.Tensor:
    tgt = torch.tensor([[BOS_IDX]], dtype=torch.long, device=src.device)  # [1,1]
...
def beam_search_decode(model: Transformer, src: torch.Tensor, max_len: int, beam_size: int) -> torch.Tensor:
    beams = [(torch.tensor([[BOS_IDX]], device=src.device, dtype=torch.long), 0.0, False)]
    for _ in range(max_len - 1):
        cand = []
        for ids, score, done in beams:
            ...
            for lp, tok in zip(topk.values.tolist(), topk.indices.tolist()):
                if ids.size(0) == 1 and tok in (BOS_IDX, EOS_IDX):  # avoid empty output
                    continue
                tok_t = torch.tensor([[tok]], device=src.device, dtype=torch.long)
                ids_new = torch.cat([ids, tok_t], dim=0)
                cand.append((ids_new, score + lp, tok == EOS_IDX))
        if not cand: break
