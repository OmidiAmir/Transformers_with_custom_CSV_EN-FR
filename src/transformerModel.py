...
from dataloader import SRC_PAD_IDX, TGT_PAD_IDX
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        if mask is not None:
            # mask: bool with True=keep, False=mask (broadcastable to [B,H,Tq,Tk])
...
def make_key_padding_keep_mask(seq_TB: torch.Tensor, pad_idx: int):
    # seq_TB: [T,B] -> keep mask [B,1,1,T]
    return (seq_TB.transpose(0, 1) != pad_idx).unsqueeze(1).unsqueeze(1)
...
class Transformer(nn.Module):
    ...
    def forward(self, src, tgt):
        # Build masks
        src_key_keep = make_key_padding_keep_mask(src, self.src_pad_idx)          # [B,1,1,Tsrc]
        Tt = tgt.size(0)
        tgt_time_keep = generate_subsequent_mask(Tt, device=tgt.device)           # [1,1,Tt,Tt]
        tgt_key_keep = make_key_padding_keep_mask(tgt, self.tgt_pad_idx)          # [B,1,1,Tt]
        tgt_keep = tgt_time_keep & tgt_key_keep                                   # broadcast -> [B,1,Tt,Tt]

        # Encode / Decode
        memory = self.encode(src, src_mask=src_key_keep)
        out = self.decode(tgt, memory, tgt_mask=tgt_keep, memory_mask=src_key_keep)  # [B,T,V]
