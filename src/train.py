...
for epoch in range(config.num_epochs):
    ...
    with torch.no_grad():
        for j, (src, tgt) in enumerate(val_dataloader):
            ...
            mask = (tgt[1:, :] != PAD_IDX)
            all_preds.append(pred_ids[mask]); all_tgts.append(tgt[1:, :][mask])
            for b in range(tgt.size(1)):
                ...
                hyp = [fr_tok.id_to_token(int(idx))
                       for idx in pred_ids[:, b] if int(idx) not in (PAD_IDX, EOS_IDX)]
                val_references.append([ref]); val_hypotheses.append(hyp)

    val_loss_avg = (val_loss / max(val_steps, 1))
    val_ppl = math.exp(val_loss_avg) if val_loss_avg < 20 else float("inf")
    if all_preds: all_preds = torch.cat(all_preds)
    if all_tgts:  all_tgts = torch.cat(all_tgts)
    val_token_acc = float((all_preds == all_tgts).float().mean().item()) if all_preds.numel() else 0.0
...
# # ================== Data ==================
# train_dataset = TranslationDataset("train")  # was "trian"
# train_dataloader = DataLoader(
#     train_dataset, batch_size=config.trainBatchSize, shuffle=True, collate_fn=collate_fn
# )
# val_dataset = TranslationDataset("val")
...
#         # grad clip (uses config.grad_clip)
#         torch.nn.utils.clip_grad_norm_(model.parameters(), config.grad_clip)
...
# print("Device is:", config.deavice)
